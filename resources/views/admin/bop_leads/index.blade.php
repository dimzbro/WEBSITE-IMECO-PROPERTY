...
<div class="space-y-8 animate-fade-in pb-16">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-1">
                Pencatatan, pemantauan status follow-up, dan rekapitulasi matriks peminat sewa ruang kantor BOP per bulan
            </p>
        </div>
    ...
    {{-- ── Flash Messages ────────────────────────────────────────────────── --}}
    @if(session('success'))
        <div class="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold rounded-2xl flex items-center gap-3 shadow-sm animate-fade-in">
            <div class="w-7 h-7 rounded-xl bg-emerald-500 text-white flex items-center justify-center flex-shrink-0">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-[0_4px_20px_rgb(0,0,0,0.04)] overflow-hidden">
        <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">Masukkan data calon penyewa ruang kantor BOP dan status follow-up awal</p>
            </div>
            <span class="px-3 py-1 bg-blue-50 text-[#1E3A8A] text-[11px] font-bold rounded-lg border border-blue-200/60">
                Pencatatan Baru
            </span>
        </div>
        ...
        <form action="{{ route('admin.bop-leads.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    <label for="form_tanggal_entry" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                ...
                {{-- Nama Peminat --}}
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Nama Peminat <span class="text-rose-500">*</span></label>
                    <input type="text" name="nama" required value="{{ old('nama') }}" placeholder="Contoh: Ririk Suriyana" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] focus:ring-2 focus:ring-blue-100 transition-all outline-none">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                {{-- Alamat Email --}}
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Alamat Email <span class="text-rose-500">*</span></label>
                    <input type="email" name="email" required value="{{ old('email') }}" placeholder="Contoh: ririksuriyana@gmail.com" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] focus:ring-2 focus:ring-blue-100 transition-all outline-none">
                </div>

                {{-- Nama Perusahaan --}}
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Nama Perusahaan (PT/CV) <span class="text-rose-500">*</span></label>
                    <input type="text" name="nama_perusahaan" required value="{{ old('nama_perusahaan') }}" placeholder="Contoh: PT Surya Nusantara" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] focus:ring-2 focus:ring-blue-100 transition-all outline-none">
                </div>

                {{-- Telpon & FAX --}}
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Telpon & FAX <span class="text-rose-500">*</span></label>
                    <input type="text" name="telpon_fax" required value="{{ old('telpon_fax') }}" placeholder="Contoh: 0812-3456-7890 / (021) 555-1234" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] focus:ring-2 focus:ring-blue-100 transition-all outline-none">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                {{-- Alamat --}}
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Alamat Perusahaan / Pemohon <span class="text-rose-500">*</span></label>
                    <textarea name="alamat" required rows="2" placeholder="Alamat lengkap lokasi atau domisili perusahaan..." class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] focus:ring-2 focus:ring-blue-100 transition-all outline-none">{{ old('alamat') }}</textarea>
                </div>

                {{-- Kategori yang Diminati --}}
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Kategori Yg Diminati (Detail Space) <span class="text-rose-500">*</span></label>
                    <textarea name="kategori_diminati" required rows="2" placeholder="Contoh: Penawaran Space: Building A; 2nd floor 130 sqm, 3rd floor 90 sqm, Building C..." class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] focus:ring-2 focus:ring-blue-100 transition-all outline-none">{{ old('kategori_diminati') }}</textarea>
                </div>
            </div>
            ...
            <div class="pt-3 border-t border-slate-100">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
                    <div>
                        ...
                        <input type="date" name="kit_marketing" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] outline-none">
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-slate-600 mb-1">LOO (Tanggal)</label>
                        <input type="date" name="loo" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] outline-none">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="date" name="dp" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] outline-none">
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-slate-600 mb-1">Serah Terima</label>
                        <input type="date" name="serah_terima" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] outline-none">
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-slate-600 mb-1">Fitting Out</label>
                        <input type="date" name="fitting_out" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-800 focus:bg-white focus:border-[#1E3A8A] outline-none">
                    </div>
                </div>
            </div>

            <div class="flex justify-end pt-2">
                <button type="submit" class="px-6 py-3 bg-[#1E3A8A] hover:bg-[#152a65] text-white font-bold text-xs rounded-xl shadow-md hover:shadow-lg transition-all cursor-pointer flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-6 border-b border-slate-100 space-y-4 relative z-20">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">Tabel rekapitulasi peminat per bulan untuk Tahun {{ $selectedYear }}</p>
                </div>
            ...
            <form action="{{ route('admin.bop-leads.index') }}" method="GET" class="flex flex-wrap items-center justify-between gap-4 pt-1 w-full">
                ...
                <div class="flex flex-wrap items-center gap-3 w-full">
                    @if($viewMode === 'list')
                        ...
                        <div class="relative cursor-pointer min-w-[150px]" onclick="toggleYearPicker('matrix')">
                            ...
                            <div id="yearPickerPopover_matrix" class="hidden absolute top-full left-0 mt-2 w-full min-w-[220px] max-w-[250px] bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 p-4 animate-fade-in">
                                <div class="flex items-center justify-between pb-3 mb-3 border-b border-slate-100">
                                    <button type="button" onclick="navigateDecade('matrix', -1); event.stopPropagation();" class="p-1 rounded-lg hover:bg-slate-100 text-slate-600 transition-colors">
                                        <svg class="w-4 h-4 text-[#1E3A8A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                                        </svg>
                                    </button>
                                    <span id="decadeTitle_matrix" class="text-xs font-black text-[#1E3A8A]">2004 – 2015</span>
                                    <button type="button" onclick="navigateDecade('matrix', 1); event.stopPropagation();" class="p-1 rounded-lg hover:bg-slate-100 text-slate-600 transition-colors">
                                        <svg class="w-4 h-4 text-[#1E3A8A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div id="yearGrid_matrix" class="flex flex-col gap-2.5 max-h-[300px] overflow-y-auto pr-1.5 gold-scrollbar"></div>
                            </div>
                        </div>
                        ...
                        <div class="relative cursor-pointer min-w-[170px]" onclick="toggleMonthPicker('matrix')">
                            ...
                            <div id="monthPickerPopover_matrix" class="hidden absolute top-full left-0 mt-2 w-full min-w-[220px] max-w-[250px] bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 p-4 animate-fade-in">
                                <div class="pb-3 mb-3 border-b border-slate-100 text-center">
                                    <span class="text-xs font-black text-[#1E3A8A]">Pilih Bulan</span>
                                </div>
                                <div id="monthGrid_matrix" class="flex flex-col gap-2.5 max-h-[300px] overflow-y-auto pr-1.5 gold-scrollbar">
                                    <button type="button" onclick="selectMonth('matrix', '', 'Semua Bulan'); event.stopPropagation();"
                                    ...
                                    @foreach($listBulan as $num => $nama)
                                        <button type="button" onclick="selectMonth('matrix', {{ $num }}, '{{ $nama }}'); event.stopPropagation();"
                        ...
                        <button type="submit" class="px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white font-semibold text-xs rounded-xl shadow-xs transition-all cursor-pointer">
                            Terapkan
                        </button>
                    @else
                    ...
                    @if($search || $selectedMonth)
                        <a href="{{ route('admin.bop-leads.index', ['tahun' => $selectedYear, 'view' => $viewMode]) }}" class="text-xs font-semibold text-rose-600 hover:underline flex items-center gap-1">
                            Reset Filter
                        </a>
                    @endif
                </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs border-collapse border border-slate-300">
                <thead>
                    ...
                    <tr style="background-color: #1E3A8A !important; color: #ffffff !important;" class="font-extrabold uppercase text-[11px] tracking-wider text-center">
                        <th rowspan="2" class="p-2.5 border border-blue-900 w-12" style="background-color: #1E3A8A !important; color: #ffffff !important;">NO.</th>
                    ...
                    {{-- Sub Header Row --}}
                    <tr style="background-color: #1E3A8A !important; color: #ffffff !important;" class="font-extrabold uppercase text-[10px] tracking-wider text-center">
                        <th class="p-2 border border-blue-900 min-w-[140px]" style="background-color: #1E3A8A !important; color: #ffffff !important;">NAMA PERUSAHAAN</th>
                        <th class="p-2 border border-blue-900 min-w-[160px]" style="background-color: #1E3A8A !important; color: #ffffff !important;">ALAMAT</th>
                ...
                <tbody>
                    ...
                    @forelse($allRecordsList as $idx => $lead)
                        <tr class="hover:bg-amber-50/40 transition-all border-b border-slate-200 text-slate-800">
                            ...
                            <td class="p-2.5 border border-slate-300 align-top">
                                ...
                                @if($lead->email)
                                    <div class="text-[11px] text-slate-600 underline mt-0.5 break-all">
                                        Email: <a href="mailto:{{ $lead->email }}" class="hover:text-blue-700">'{{ $lead->email }}'</a>
                                    </div>
                                @else
                            ...
                            {{-- TELPON & FAX --}}
                            <td class="p-2.5 border border-slate-300 align-top text-slate-700 whitespace-pre-line text-[11px]">
                            ...
                            <td class="p-2 border border-slate-300 align-top text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    <form action="{{ route('admin.bop-leads.destroy', $lead->id) }}" method="POST" onsubmit="return confirm('Hapus data peminat ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                                                title="Hapus"
                    ...
                    @forelse($records as $index => $r)
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="px-6 py-4 font-extrabold text-slate-900 whitespace-nowrap">
                                <div class="text-xs font-black text-slate-900">{{ formatFollowUpDate($r->tanggal_entry) }}</div>
                                <div class="text-[10px] text-slate-400 font-semibold mt-0.5">{{ $r->nama_bulan }} {{ $r->tahun }}</div>
                            </td>
                            ...
                            <td class="px-4 py-4 text-center">
                                <div class="inline-flex flex-col gap-1 text-[10px] text-left">
                                    <div class="flex items-center gap-1.5">
                                        <span class="font-bold text-slate-400 w-7">Kit:</span>
                                        @if($r->kit_marketing)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-700 border border-slate-200">{{ formatFollowUpDate($r->kit_marketing) }}</span>
                                        @else
                                    ...
                                    <div class="flex items-center gap-1.5">
                                        <span class="font-bold text-emerald-600 w-7">DP:</span>
                                        @if($r->dp)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">{{ formatFollowUpDate($r->dp) }}</span>
                                        @else
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-400">-</span>
                                        @endif
                                    </div>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    {{-- Hapus Data Icon --}}
                                    <form action="{{ route('admin.bop-leads.destroy', $r->id) }}" method="POST" class="inline"
...
{{-- ── Edit Modal ────────────────────────────────────────────────────────── --}}
<div id="editModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center hidden p-4 animate-fade-in">
    <div class="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200 p-6 space-y-5">
        ...
        <form id="editForm" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Nama Peminat <span class="text-rose-500">*</span></label>
                    <input type="text" name="nama" id="edit_nama" required class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:border-[#1E3A8A]">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="email" name="email" id="edit_email" required class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:border-[#1E3A8A]">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Nama Perusahaan <span class="text-rose-500">*</span></label>
                    <input type="text" name="nama_perusahaan" id="edit_nama_perusahaan" required class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:border-[#1E3A8A]">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Telpon & FAX <span class="text-rose-500">*</span></label>
                    <input type="text" name="telpon_fax" id="edit_telpon_fax" required class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:border-[#1E3A8A]">
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Alamat <span class="text-rose-500">*</span></label>
                    <textarea name="alamat" id="edit_alamat" required rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:border-[#1E3A8A]"></textarea>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Kategori Yg Diminati <span class="text-rose-500">*</span></label>
                    <textarea name="kategori_diminati" id="edit_kategori_diminati" required rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:border-[#1E3A8A]"></textarea>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-2 pt-3">
                <button type="button" onclick="closeEditModal()" class="px-4 py-2 bg-slate-100 text-slate-700 font-bold text-xs rounded-xl hover:bg-slate-200 transition-all cursor-pointer">
                    Batal
                </button>
                <button type="submit" class="px-5 py-2 bg-[#1E3A8A] text-white font-bold text-xs rounded-xl shadow hover:bg-[#152a65] transition-all cursor-pointer">
                    Simpan Perubahan
                </button>
            </div>
        </form>
...
<div id="quickFollowUpModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center hidden p-4 animate-fade-in">
    <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-200 p-6 space-y-4">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <button type="button" onclick="closeQuickFollowUpModal()" class="text-slate-400 hover:text-slate-600 font-bold text-lg">&times;</button>
        </div>
        ...
        <form id="quickFollowUpForm" method="POST" onsubmit="submitQuickFollowUp(event)" class="space-y-4">
            ...
            <div>
                <label id="quickFieldLabel" class="block text-xs font-bold text-slate-700 mb-1.5">Tanggal Follow Up</label>
                <input type="date" id="quick_field_value" name="value" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 outline-none focus:border-[#1E3A8A] focus:bg-white">
            ...
            <div id="quickNomorSuratLooContainer" class="hidden">
                ...
                <input type="text" id="quick_nomor_surat_loo" name="nomor_surat_loo" placeholder="Contoh: 001/LOO/IMECO/2026" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 outline-none focus:border-[#1E3A8A] focus:bg-white">
                <p class="text-[11px] text-slate-400 mt-1">Isi nomor surat resmi Letter of Offer (LOO).</p>
            </div>

            <div class="flex justify-end gap-2 pt-2">
                <button type="button" onclick="closeQuickFollowUpModal()" class="px-4 py-2 bg-slate-100 text-slate-700 font-bold text-xs rounded-xl hover:bg-slate-200 transition-all cursor-pointer">
                    Batal
                </button>
                <button type="submit" class="px-5 py-2 bg-[#1E3A8A] text-white font-bold text-xs rounded-xl shadow hover:bg-[#152a65] transition-all cursor-pointer">
                    Update Matriks
    ...
    function submitQuickFollowUp(e) {
        ...
        if (field === 'loo') {
            ...
            if (looVal && !noSuratVal) {
                alert('Jika Tanggal LOO diisi, maka Nomor Surat LOO juga wajib diisi.');
                return;
            }
            if (!looVal && noSuratVal) {
                alert('Jika Nomor Surat LOO diisi, maka Tanggal LOO juga wajib diisi.');
                return;
            }
        }
