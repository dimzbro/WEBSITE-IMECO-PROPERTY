...
@section('title', 'Penggunaan Daya Listrik – Admin Portal')
@section('breadcrumb', 'Penggunaan Daya Listrik')
...
<div class="space-y-8 animate-fade-in pb-16">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-1">
                Pencatatan, pemantauan, dan analisis penggunaan energi listrik (KWH) setiap gedung per bulan
            </p>
        </div>

        @if($records->total() > 0)
        <div class="flex items-center gap-2">
            <form action="{{ route('admin.electricity.destroyAll') }}" method="POST"
                  onsubmit="return confirm('Apakah Anda yakin ingin menghapus data penggunaan listrik sesuai filter aktif? Tindakan ini tidak dapat dibatalkan.');">
                @csrf
                @method('DELETE')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div class="p-5 bg-white rounded-2xl border border-slate-200/80 shadow-[0_4px_20px_rgb(0,0,0,0.04)] flex flex-col justify-between">
            ...
            <div class="mt-4">
                <p class="text-2xl font-black text-slate-900 tracking-tight">{{ number_format($totalKwhSelectedYear, 2, ',', '.') }}</p>
                <p class="text-[11px] font-semibold text-slate-500 mt-0.5">KWH {{ $selectedGedung ? "($selectedGedung)" : '(Semua Gedung)' }}</p>
            </div>
        </div>
        ...
        <div class="p-5 bg-white rounded-2xl border border-slate-200/80 shadow-[0_4px_20px_rgb(0,0,0,0.04)] flex flex-col justify-between">
            ...
            <div class="mt-4">
                <p class="text-2xl font-black text-slate-900 tracking-tight">{{ number_format($avgKwhPerMonth, 2, ',', '.') }}</p>
                <p class="text-[11px] font-semibold text-slate-500 mt-0.5">KWH / Bulan Terdaftar</p>
            </div>
        </div>
        ...
        <div class="p-5 bg-white rounded-2xl border border-slate-200/80 shadow-[0_4px_20px_rgb(0,0,0,0.04)] flex flex-col justify-between">
            ...
            <div class="mt-4">
                @if($highestMonth)
                    <p class="text-2xl font-black text-slate-900 tracking-tight">{{ number_format($highestMonth->total_kwh, 2, ',', '.') }}</p>
        ...
        <div class="p-5 bg-white rounded-2xl border border-slate-200/80 shadow-[0_4px_20px_rgb(0,0,0,0.04)] flex flex-col justify-between">
            ...
            <div class="mt-4">
                @if($lowestMonth)
                    <p class="text-2xl font-black text-slate-900 tracking-tight">{{ number_format($lowestMonth->total_kwh, 2, ',', '.') }}</p>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-[0_4px_20px_rgb(0,0,0,0.04)] p-6 lg:p-8">
        <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
            ...
            <div>
                <h3 class="text-base font-extrabold text-slate-900">Form Input Penggunaan Daya Listrik</h3>
                <p class="text-xs text-slate-500 mt-0.5">Masukkan data penggunaan energi listrik (KWH) gedung setiap bulan</p>
            </div>
        </div>

        <form action="{{ route('admin.electricity.store') }}" method="POST" class="space-y-6">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
                ...
                {{-- Field KWH --}}
                <div class="space-y-1.5">
                    <label for="form_kwh" class="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                        Penggunaan Daya (KWH) <span class="text-rose-500">*</span>
                    </label>
                    <input type="number" step="0.01" min="0.01" id="form_kwh" name="kwh" required
                           value="{{ old('kwh') }}"
                           placeholder="Contoh: 90500"
                           class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#1E3A8A] transition-all">
                    @error('kwh')
                        <p class="text-[11px] font-bold text-rose-500">{{ $message }}</p>
                    @enderror
    ...
    {{-- ── 3. Tabel Rekapitulasi Penggunaan Daya Listrik (Matriks Bulanan & Log) ─ --}}
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-[0_4px_20px_rgb(0,0,0,0.04)]">
        ...
        <div class="p-6 border-b border-slate-100 space-y-4 relative z-20">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h3 class="text-base font-extrabold text-slate-900">Rekapitulasi Penggunaan Daya Listrik (KWH)</h3>
                    <p class="text-xs text-slate-500 mt-0.5">Tabel rekapitulasi energi per gedung per bulan untuk Tahun {{ $selectedYear }}</p>
                </div>

                {{-- Mode Switcher & Controls --}}
                <div class="flex items-center gap-3">
                    <div class="bg-slate-100 p-1 rounded-xl flex items-center gap-1 border border-slate-200/60">
                        <a href="{{ route('admin.electricity.index', array_merge(request()->query(), ['view' => 'matrix'])) }}" 
                        ...
                        <a href="{{ route('admin.electricity.index', array_merge(request()->query(), ['view' => 'list'])) }}" 
            ...
            @if($viewMode === 'matrix')
                ...
                <form id="filterForm_matrix" action="{{ route('admin.electricity.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
            ...
            @else
                ...
                <form action="{{ route('admin.electricity.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                    <input type="hidden" name="view" value="list">

                    {{-- Search Box --}}
                    <div class="relative flex items-center">
                        <input type="text" name="search" value="{{ $search }}" placeholder="Cari gedung, ID, KWH..."
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if($search || $selectedMonth || $selectedGedung || $selectedYear != date('Y'))
                            <a href="{{ route('admin.electricity.index', ['view' => $viewMode]) }}" class="px-3 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-xs rounded-xl transition-all" title="Reset Filter">
                                Reset
                            </a>
                        @endif
                    </div>
            ...
            <table class="w-full border-collapse border border-slate-300 text-xs font-medium">
                <thead>
                    ...
                    <tr>
                        <th rowspan="2" 
                            ...
                            class="border border-blue-900 px-4 py-3 text-center align-middle font-black text-sm uppercase tracking-wider min-w-[180px]">
                            GEDUNG
                        </th>
                        <th rowspan="2" 
                            style="background-color: #1E3A8A !important; color: #ffffff !important;"
                            class="border border-blue-900 px-4 py-3 text-center align-middle font-black text-sm uppercase tracking-wider min-w-[150px]">
                            NOMOR ID
                        </th>
                        <th colspan="12" 
                            style="background-color: #1E3A8A !important; color: #ffffff !important;"
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    @foreach($listGedung as $gedungName)
                        ...
                        <tr class="hover:bg-sky-50/50 transition-colors">
                            ...
                            {{-- Nomor ID Gedung --}}
                            <td class="border border-slate-300 px-4 py-3 font-bold text-slate-600 bg-slate-50/30 text-center font-mono whitespace-nowrap select-all">
                                {{ $nomorIdMapping[$gedungName] ?? '-' }}
                            </td>
                            ...
                            @for($m = 1; $m <= 12; $m++)
                                ...
                                <td class="border border-slate-300 px-3 py-2.5 text-right font-bold transition-all cursor-pointer group hover:bg-blue-100/70"
                                    @if($cell)
                                        ...
                                        title="{{ $gedungName }} (ID: {{ $nomorIdMapping[$gedungName] ?? '-' }}) - {{ $shortMonthNames[$m-1] }}-{{ $shortYear }}: {{ number_format($cell['kwh'], 2, ',', '.') }} KWH (Klik untuk detail)"
                                    @else
                                        onclick="prefillForm('{{ addslashes($gedungName) }}', {{ $selectedYear }}, {{ $m }})"
                                        title="{{ $gedungName }} (ID: {{ $nomorIdMapping[$gedungName] ?? '-' }}) - {{ $shortMonthNames[$m-1] }}-{{ $shortYear }}: Data Belum Ada (Klik untuk menginput)"
                                    @endif>
                                    
                                    @if($cell)
                                        <span class="text-slate-900 group-hover:text-[#1E3A8A]">
                                            {{ number_format($cell['kwh'], 0, ',', '.') }}
                                        </span>
                                    @else
                ...
                <tfoot>
                    <tr class="bg-slate-100/90 font-extrabold border-t-2 border-slate-300 text-slate-800">
                        <td colspan="2" class="border border-slate-300 px-4 py-3 text-center uppercase tracking-wider bg-slate-200/80">
                            TOTAL KWH
                        </td>
                        @for($m = 1; $m <= 12; $m++)
        ...
        <div class="p-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-semibold text-slate-500">
            <div class="flex items-center gap-2">
                ...
                Tabel Matriks Rekapitulasi Daya Listrik & Mapping Nomor ID (Tahun {{ $selectedYear }})
            </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/80 border-b border-slate-200 text-[11px] font-black text-slate-500 uppercase tracking-wider">
                        ...
                        <th class="px-6 py-4 text-right">Penggunaan (KWH)</th>
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    {{-- Hapus --}}
                                    <form action="{{ route('admin.electricity.destroy', $item->id) }}" method="POST" class="inline"
                        ...
                        <tr>
                            <td colspan="10" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center space-y-3">
                                    ...
                                    <p class="text-sm font-bold text-slate-600">Belum ada data penggunaan daya listrik</p>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-[0_4px_20px_rgb(0,0,0,0.04)] p-6 lg:p-8">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-100">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">
                    Tren penggunaan energi bulanan: Tahun <span class="font-bold text-[#1E3A8A]">{{ $selectedYear }}</span> vs Tahun <span class="font-bold text-amber-600">{{ $previousYear }}</span>
                </p>
            </div>
        ...
        <div class="relative w-full h-[500px]">
            <canvas id="electricityLineChart"></canvas>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full max-w-lg overflow-hidden transform transition-all">
        <div class="p-6 bg-slate-900 text-white flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-extrabold text-sm" id="detail_title">Detail Penggunaan Daya</h3>
                    <p class="text-[11px] text-slate-400" id="detail_subtitle">Rincian data energi</p>
                </div>
            </div>
        ...
        <div class="p-6 space-y-4">
            ...
            <div class="p-5 bg-blue-50/60 rounded-2xl border border-blue-100 text-center">
                <span class="text-[10px] font-extrabold text-blue-900 uppercase tracking-wider block">Penggunaan Daya Listrik</span>
                <span class="text-3xl font-black text-[#1E3A8A] mt-1 block" id="detail_kwh">- KWH</span>
            </div>
            ...
            <div class="space-y-3 pt-2">
                ...
                <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2 text-xs">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="font-extrabold text-slate-800" id="detail_prev_kwh">-</span>
                    </div>
                    <div class="flex justify-between items-center">
    ...
    <div class="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full max-w-lg overflow-hidden transform transition-all">
        <div class="p-6 bg-slate-900 text-white flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-extrabold text-sm">Edit Data Penggunaan Daya</h3>
                    <p class="text-[11px] text-slate-400">Perbarui rekaman penggunaan listrik</p>
                </div>
            </div>
        ...
        <form id="editForm" method="POST" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5 relative">
                    ...
                    <div id="yearPickerPopover_edit" class="hidden absolute top-full left-0 mt-2 w-full min-w-[240px] bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 p-4 animate-fade-in">
                        <div class="flex items-center justify-between pb-3 mb-3 border-b border-slate-100">
                            <button type="button" onclick="navigateDecade('edit', -1)" class="p-1.5 rounded-lg hover:bg-slate-100 text-slate-600 transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                                </svg>
                            </button>
            ...
            <div class="space-y-1.5">
                <label for="edit_kwh" class="block text-xs font-bold text-slate-700 uppercase tracking-wider">Penggunaan Daya (KWH)</label>
                <input type="number" step="0.01" min="0.01" id="edit_kwh" name="kwh" required
                       placeholder="Contoh: 90500"
                       class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#1E3A8A]">
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const ctx = document.getElementById('electricityLineChart');
    if (ctx) {
...
function prefillForm(gedung, tahun, bulan) {
    ...
    const formKwh = document.getElementById('form_kwh');
    if (formKwh) formKwh.focus();
...
function openDetailModal(item) {
    ...
    let kwhFormat = new Intl.NumberFormat('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(item.kwh);
    document.getElementById('detail_kwh').textContent = kwhFormat + ' KWH';

    let prevYear = item.tahun - 1;
    document.getElementById('detail_prev_year_label').textContent = 'Penggunaan Tahun ' + prevYear + ':';

    if (item.prev_kwh !== null && item.prev_kwh !== undefined) {
        let prevFormat = new Intl.NumberFormat('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(item.prev_kwh);
        document.getElementById('detail_prev_kwh').textContent = prevFormat + ' KWH';
        ...
        document.getElementById('detail_selisih').textContent = prefix + selisihFormat + ' KWH';
        ...
        document.getElementById('detail_prev_kwh').textContent = '-';
...
function openEditModalFromObject(item) {
    ...
    form.action = "{{ url('admin/electricity') }}/" + item.id;
    ...
    document.getElementById('edit_kwh').value = item.kwh;
